...
def csv_generator(value_name='total_energy',
                  ...
                  input_txt="test.txt", output_csv="outfile_ult.csv"):
    """
    :param value_name: either energy or power
    :param workload_type_name:
    :param input_txt:
    :param output_csv:
    :return:
    """
    with open(input_txt) as infile, open(output_csv, "w") as outfile:
        ...
        if value_name == 'average_power':
            layer_list = re.findall(r"" + workload_type_name + '_(.*?).txt:average_power',
                                         data)
            ...
            layer_list = re.findall(r"" + workload_type_name + '_(.*?).txt:total_energy',
                                    data)
            for i in range(len(layer_list)):
                layer_value += re.findall(
                    r"" + workload_type_name + "_" + str(layer_list[i]) +
